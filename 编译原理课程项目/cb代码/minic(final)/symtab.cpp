...
struct SYMBOL
{
    ...
    void SYMBOLPrint()
    {
        printf("name:%-6s\ttype:%-5s\t%-5s\tisArray:%-2d\tline:%-3dlocations:", this->name, typeToString(this->type), (this->isID)?"ID":"FUNC", this->isArray, this->line);
        for (set<int>::iterator it = locations.begin(); it != locations.end(); it++) printf("%-3d", *it);
...
//            }
    ...
    case StmtK:
        ...
        case CallK:
            ...
            else printf("line %d: %s is used but not be defined.\n", t->lineno, t->attr.name);
            break;
        default:
            break;
        }
